...
class Transaction {
  ...
  createRawTransaction = async (param) => {
    const result = await Parse.Cloud.run('createRawTransaction', param);
  ...
  sendSignedTransaction = async (params) => {
    const result = await Parse.Cloud.run('sendSignedTransaction', params);
    this.txHash = getTxHash(this.symbol, result);
    return result;
  }

  setTxHash(transaction) {
    this.txHash = getTxHash(this.symbol, transaction);
  }
  ...
  async processSignedTransaction() {
    ...
    if (this.signedTransaction) {
      try {
        ...
        result = await this.sendSignedTransaction(param);
